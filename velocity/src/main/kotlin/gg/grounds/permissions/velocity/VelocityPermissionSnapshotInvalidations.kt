...
internal fun interface VelocityInvalidationSubscriberFactory {
    fun subscribe(
...
internal class VelocityPermissionSnapshotInvalidations
private constructor(
        ...
        try {
            ...
            logger.warn(
                "Failed to close permission snapshot invalidation subscriber " +
                    "(runtime=velocity, subject={}, reason=subscriber_close_failed)",
                subject,
            )
        }
        try {
            ...
            logger.warn(
                "Failed to close permission snapshot invalidation executor " +
                    "(runtime=velocity, subject={}, reason=executor_close_failed)",
                subject,
            )
        }
    }
    ...
    companion object {
        fun start(
            ...
            proxy: ProxyServer,
            logger: Logger,
        ): VelocityPermissionSnapshotInvalidations? =
            start(
                config = config,
                snapshots = snapshots,
                runtimeClient = runtimeClient,
                context = context,
                isOnline = { playerId -> proxy.getPlayer(playerId).isPresent },
                logger = logger,
            )
        ...
        internal fun start(
            ...
            logger: Logger,
            subscriberFactory: VelocityInvalidationSubscriberFactory = DEFAULT_SUBSCRIBER_FACTORY,
                ...
                try {
                    ...
                    logger.warn(
                        "Failed to start permission snapshot invalidation executor " +
                            "(runtime=velocity, subject={}, reason=executor_start_failed)",
                        config.subject,
                    )
                    return null
                }
                ...
                try {
                    ...
                    logger.warn(
                        "Failed to start permission snapshot invalidation subscriber " +
                            "(runtime=velocity, subject={}, reason=subscriber_start_failed)",
                        config.subject,
                    )
                    return null
                }
            return VelocityPermissionSnapshotInvalidations(
        ...
        private val DEFAULT_SUBSCRIBER_FACTORY =
            VelocityInvalidationSubscriberFactory { config, logger, handler ->
